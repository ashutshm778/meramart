                        ...
                        <div class="ec-vendor-card-body">
                            <div class="ec-vendor-card-table">
                                <table class="table ec-table">
                                    ...
                                    <tbody>
                                        @php
                                            $order_histories = App\Models\Order::where('frenchies_code',Auth::guard('customer')->user()->referral_code)->orderby('id','desc')->paginate(10);
                                        @endphp
                                        @foreach ($order_histories as $order_history)
                                            <tr>
                                                ...
                                                <td><span>{{ucFirst($order_history->order_status)}}</span></td>
                                                <td><span>{{ucFirst($order_history->payment_status)}}</span></td>
                                                <td>
                                                    <span class="tbl-btn"><a class="btn btn-lg btn-primary" href="{{route('order_history_details',$order_history->id)}}">View</a></span>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
