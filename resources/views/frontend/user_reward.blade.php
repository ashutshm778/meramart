        ...
        if (!empty($customer->referral_code)) {
            $customer_data =  App\Models\Customer::where('refered_by', $customer->referral_code)->get();
                ...
                foreach ($customer_data as $customer_referral) {
                    if (empty($one_side) && (($customer_referral->group_pv) >= $one_side_count)) {
                ...
                foreach (App\Models\Customer::where('refered_by', $customer->referral_code)->where('id', '!=', $one_side)->get() as $customer_referrals) {
                    if (!empty($one_side) && ($other_side < $other_side_count)) {
                        $other_side = $other_side + ($customer_referrals->group_pv);
                        array_push($other_side_id,['customer_id'=>$customer_referrals->id,'group_pv'=>($customer_referrals->group_pv)] );
                    }
                }
                if ($other_side >= $other_side_count) {
                    $one_side_customer=App\Models\Customer::where('id',$one_side)->first();
                    $one_side_customer->group_pv=$one_side_customer->group_pv-$one_side_count;
                    ...
                    foreach($other_side_id as $otherSideId){
                        ...
                        if($otherSideId['group_pv'] >= $other_side_count){
                            $other_side_customer->group_pv=$other_side_customer->group_pv-$other_side_count;
                        }else{
                            $other_side_customer->group_pv=$other_side_customer->group_pv-$otherSideId['group_pv'];
                        }
