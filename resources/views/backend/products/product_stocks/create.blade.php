                                            ...
                                            <div class="row">
                                                <div class="col-md-4 form_div">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="vendor" class="form-control select2" id="vendor" onchange="get_product()">
                                                            <option value="">Select Vendor...</option>
                                                            @foreach (App\Models\Admin\Vendor::orderBy('business_name')->get() as $vendor)
                                                                <option value="{{$vendor->id}}">{{$vendor->business_name}} ({{$vendor->phone}})</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-md-4 form_div" style="margin-top:30px">
                                                    <div class="form-group">
...
<script>
    ...
    function get_product_table()
    {
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('admin.get.product.table')}}",
            data:{
                products:productss
            },
            success: function(data) {
    ...
    function save_offer_product()
    {
        if(validate_form())
        {
            ...
            var purchase_price = $("input[name='purchase_prices[]']").map(function(){return $(this).val();}).get();
            var mrp_price = $("input[name='mrp_prices[]']").map(function(){return $(this).val();}).get();
            var stockes = $("input[name='stockes[]']").map(function(){return $(this).val();}).get();
            var retailer_selling_price = $("input[name='retailer_selling_price[]']").map(function(){return $(this).val();}).get();
            var distributor_selling_price = $("input[name='distributor_selling_price[]']").map(function(){return $(this).val();}).get();
            var wholeseller_selling_price = $("input[name='wholeseller_selling_price[]']").map(function(){return $(this).val();}).get();
            ...
            $.ajax({
                type: 'POST',
                url: "{{route('admin.product-stocks.store')}}",
                data:{
                    purchase_price:purchase_price,
                    mrp_price:mrp_price,
                    retailer_selling_price:retailer_selling_price,
                    distributor_selling_price:distributor_selling_price,
                    wholeseller_selling_price:wholeseller_selling_price,
                    ids:ids,
                    vendor:vendor,
                    stockes:stockes
                },
                success: function(data) {
                    localStorage.clear();
                    window.location.href = "{{route('admin.product-stocks.index')}}";
                }
            });
        }
        else
